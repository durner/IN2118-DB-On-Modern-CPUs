...
static void* scan(void* arg)
{
    ...
    while (!stop) {
        ...
        for (unsigned page = start; page < start + 10; page++) {
            CBufferFrame& bf = bm->fixPage(page, false);
...
static void* readWrite(void* arg)
{
    ...
    for (unsigned i = 0; i < 100000 / threadCount; i++) {
        ...
        CBufferFrame& bf = bm->fixPage(randomPage(threadNum), isWrite);
...
int main(int argc, char** argv)
{
    ...
    bm = new CBufferManager(pagesInRAM);
    ...
    for (unsigned i = 0; i < pagesOnDisk; i++) {
        CBufferFrame& bf = bm->fixPage(i, true);
    ...
    for (unsigned i = 0; i < pagesOnDisk; i++) {
        CBufferFrame& bf = bm->fixPage(i, false);
